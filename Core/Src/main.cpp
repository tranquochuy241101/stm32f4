...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if(htim->Instance == htim5.Instance)
	{
			...
			P_control = Kp*(e - e_prev);							// Kp*(e(k) - e(k-1)
			I_control = Ki*0.01*(e + e_prev);						// Ki*T/2*(e(k) + e(k-1))
			if(cnt==1)
				u= Setpoint_value;
			else
				u = u_prev + P_control + I_control;						// next signal to motor
			e_prev = e;
			u_prev = u;
...
{
	/*if(duty ==0)
	{while(duty<99){htim2.Instance->CCR1 = ++duty;}
	if (duty == 99) return;}

	if(duty==99){
	{while(duty >0){htim2.Instance->CCR1 = --duty;}
	}}*/
}
/**
